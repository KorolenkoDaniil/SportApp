...
class RankingsActivitySoccerViewModel: ViewModel(), BaseViewModelInterface <RankingsState, SoccerRepository> {
    ...
    override fun loadData() {
        viewModelScope.launch {
            try {
                state.value = RankingsState.RankingsContent(repository.getRankings())
            }
            catch (e: Throwable){
